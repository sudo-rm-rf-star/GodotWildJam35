...
func _on_Area2D_body_entered(body: PhysicsBody2D):
	...
	_tween.remove_all()
	_tween.interpolate_property(camera, "zoom", camera.zoom, _zoom_out, .4, Tween.TRANS_BACK)
...
func _on_Area2D_body_exited(body):
	...
	_tween.remove_all()
	_tween.interpolate_property(camera, "zoom", camera.zoom, _saved_camera_zoom, .4, Tween.TRANS_BACK)
